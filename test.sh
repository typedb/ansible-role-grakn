...
if [ $distro = 'centos7' ]; then
  ...
  opts="--cap-add=SYS_ADMIN --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro"
# CentOS 6
...
docker run --detach --volume="$PWD":/etc/ansible/roles/role_under_test:rw --volume="$PWD":/etc/ansible/roles/:rw $opts geerlingguy/docker-$distro-ansible:latest $init > "$container_id"
